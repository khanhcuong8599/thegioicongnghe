...
class BrandProduct extends Controller
{
    ...
    public function unactive_brand_product(Request $request, $brand_product_id){
        ...
        $request->session()->flash('statuscode', 'success');
        return Redirect::to('all-brand-product')->with('status','Hiển thị thương hiệu sản phẩm thành công!!!');
    }
    public function active_brand_product(Request $request, $brand_product_id){
        $this->AuthLogin();
        DB::table('tbl_brand_product')->where('brand_id',$brand_product_id)->update(['brand_status'=>0]);
        $request->session()->flash('statuscode', 'success');
        return Redirect::to('all-brand-product')->with('status','Ẩn thương hiệu sản phẩm thành công!!!');
    }
    ...
    public function show_brand_home($brand_id){
        ...
        $brand_by_id = DB::table('tbl_product')->join('tbl_brand_product','tbl_product.brand_id','=','tbl_brand_product.brand_id')
            ->where('tbl_product.brand_id',$brand_id)->where('tbl_product.product_status',1)->get();
